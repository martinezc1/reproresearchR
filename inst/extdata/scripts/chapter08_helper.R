...
pizza_plot <- ggplot(___, aes(x = price, y = ___)) +
  geom_point(size = 3)  +
...
library(ggformula)
pizza_plot_ggformula <- gf_point(volume ~ price, data =___) %>% gf_lm(color ="purple")
...
## ----Correlation---------------------------------------------------------------
cor.test(___$volume,pizza_sales$___)


## ----LM------------------------------------------------------------------------
pizza_lm <- lm(volume ~ ___, data=___)
...
ggplot(pizza_sales, aes(x = predicted, y = ___)) +
...
m4 <- lm(volume ~ price + ad_spend + ___, data = pizza_sales)  # both
