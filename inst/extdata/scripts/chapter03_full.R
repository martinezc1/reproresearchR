...
## ----example-ggplot2----
ggplot(data = mpg, aes(x = ___, y = hwy)) +
  geom_point()


## ----basic-scatterplot----
ggplot(data = mpg, aes(x = cty, y = ___)) +
  geom_point()
...
ggplot(mpg, aes(x = cty, y = hwy)) +
  geom_point() +
  ___(method = "lm") # lm stands for "linear model"
...
ggplot(data = mpg, aes(x = cty, y = hwy, color = class, shape = drv)) +
  ...
  labs(
    ___ = "City vs Highway MPG",      # adds a title to the plot
  ...
  facet_wrap(~ ___) +                 # breaks the graph into individual graphs
...
ggplot(mpg, aes(x = manufacturer, fill = drv)) +
  geom_bar(position = "___", color = "white") +
  coord_flip() +
  labs(
...
# GROUPED
ggplot(___ %>% filter(cut %in% c("Good", "Ideal", "Premium")),
       aes(price, color = cut)) +
  geom_density() +
...
ggplot(mpg, aes(x = class, y = hwy)) +
  geom_boxplot() +
  ___(title = "Highway MPG by Vehicle Class", x = "Vehicle Class", y = "Highway MPG")


## ----jittered-boxplot, fig.cap= "An example of a boxplot with jittered points."----
# WITH JITTERED POINTS OVERLAID
ggplot(mpg, aes(class, hwy)) +
  ___(outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.4, size = 1.5) +
  coord_flip() +
  ___(title = "Highway MPG by Vehicle Class (with Points)",
       x = "", y = "Highway MPG")
...
ggplot(economics, aes(date, ___)) +
  ...
  labs(___ = "US Unemployment with Linear Trend",
       x = "Date", ___ = "Number Unemployed (thousands)")


## ----loess-linegraph, fig.cap= "An example of line graph with a LM line of best fit."----
ggplot(economics, aes(___, unemploy)) +
  ___(linewidth = 0.6) +
  ___(method = "loess", se = TRUE) +  # loess = flexible smoothing; se = confidence band
...
___(summ_hwy, aes(class, mean_hwy)) +
  geom_errorbar(aes(ymin = mean_hwy - se_hwy, ymax = mean_hwy + se_hwy), width = 0.2) +
  geom_point(size = 2) +
  coord_flip() +
  ___(title = "Mean Highway MPG (± SE) by Class", x = "", y = "Mean Highway MPG")
...
ggplot(___, aes(displ, hwy)) +
  ___(alpha = 0.6) +
  geom_vline(xintercept = 3, linetype = "dotted") +
  ___(title = "Reference Line at Engine Displacement = 3L",
       x = "Engine Displacement (L)", y = "Highway MPG")
